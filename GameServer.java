	...
	public static void main(String[] args)
	{
		...
		try {
			...
			while(true) {
				...
				if(needToSkip) {
					System.out.println("needToSkip = true");
				}
				else {
					System.out.println("needToSkip = false");
				}
				//DEEBUG
				...
				if(cardGame.lastPlayed().getVal().equals("skip") && needToSkip) {
					...
					for(ClientPair client : clients) {
						DataOutputStream outToYou = new DataOutputStream(client.getSocket().getOutputStream());
						outToYou.writeUTF("player " + playOrder.getPlayer().getName() + " drew2 skippe");
					}
					//Skip
					playOrder.nextPlayer();
					needToSkip = false;
				...
				if(cardGame.hasLegalPlay(playOrder.getPlayer())) {
					...
					//Check if the move was some form of skip
					String[] moveCheck = move.split(" ");
					if(moveCheck[1].equals("draw4") || moveCheck[1].equals("draw2")
						|| moveCheck[1].equals("skip")) {
							//DEBUG
							System.out.println("needToSkip = true");
							needToSkip = true;
					}
